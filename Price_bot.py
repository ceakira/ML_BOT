...
#recebe o argumento de pagina_atual para garantir que ele nao ira baixar todas as paginas de uma vez, garantindo um funcionamento mais rapido
def ScrapeAndPrint(pagina_atual): 
    ec = pyshorteners.Shortener()
    headers = {
   ...
   # produtos_por_pagina = 48
    ...
    # Encaixando tudo no zip:
            for tag_titulo, tag_link, tag_high, tag_preco, tag_disc in zip(tags_titulo, tags_links, tags_highlightML,  tags_price,  tags_discount):
                
                            titulo = tag_titulo.get_text(strip=True)
                            link = tag_link.get('href', 'Link não encontrado')
                            if link:
                                 link = ec.isgd.short(link)
